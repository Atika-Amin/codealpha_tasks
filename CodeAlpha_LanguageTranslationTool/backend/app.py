...
Also serves a text-to-speech fallback: the frontend tries the browser's own
SpeechSynthesis first (instant, free, no network), and only calls
/api/speak when the browser/OS has no installed voice for that language —
common for languages like Bengali on many systems. This uses gTTS, which
(like google_free above) talks to Google Translate's own free, unofficial
TTS endpoint — no key, no card, same trade-offs (can be rate-limited).

Endpoints:
  POST /api/translate   { text, source, target } -> { translatedText, ... }
  POST /api/speak        { text, lang } -> audio/mpeg (MP3 bytes)
  GET  /api/health       reports which translation provider is active
...
def speak():
    """
    Fallback text-to-speech, only meant to be called when the browser's own
    SpeechSynthesis has no installed voice for the requested language.
    Returns raw MP3 bytes generated by gTTS.
    """
    body = request.get_json(silent=True) or {}
    text = (body.get("text") or "").strip()
    lang = body.get("lang")
    ...
    try:
        ...
        # Most often means gTTS doesn't recognize this language code, or
        # Google's free endpoint is temporarily rate-limiting/blocking.
        return jsonify({"error": f"Text-to-speech failed: {e}"}), 502
    except requests.RequestException:
        return jsonify({"error": "Could not reach the text-to-speech service."}), 502
